...
\
&Egrave; possibile filtrare questi risultati in base al tipo di oggetto cercato: \

![](images\Etherscan_filtro.png)


Sono svolte tre prove con la barra di ricerca. Se la chiave di ricerca ha un solo risultato, Etherscan porta direttamente alla sua pagina. Se la chiave di ricerca ha molteplici risultati, Etherscan propone una pagina in cui sono elencati i risultati ottenuti. Inoltre in questa pagina viene proposto un secondo filtro. Questo permette di scegliere tra token, indirizzi e nomi di dominio. \
\
![](images\Etherscan_search_results.png)

\
Le parole chiave cercate sono *Luna*, *LUNC*, e l'indirizzo esadecimale del contratto del token LUNC. Per ognuna di queste parole sono provati tutti i filtri proposti nella barra di ricerca. Quello che emerge è che utilizzare i filtri non cambia i risultati ottenuti. Con le parole *Luna* e *LUNC* sono proposti sempre gli stessi risultati. Con l'indirizzo esadecimale Etherscan porta direttamente alla pagina del contratto. \
...
\
In alto a destra c'è il menù principale. Questo ha sei voci: *Blockchain*, *Token*, *NFTs*, *Resources*, *Developers*, *More*. Queste sei offrono dei link agli strumenti e risorse disponibili.
\
*Blockchain* offre link a risorse riguardanti transazioni e blocchi. \
![](images\Etherscan_menu_1.png)

Il link *Transactions* porta ad una pagina in cui sono elencate tutte le transazioni avvenute. Il link *Pending Transactions* collega ad una pagina in cui sono propone tutte le transazioni in attesa. Inoltre questa pagina propone un link alla pending transaction pool. \
In modo simile link *Contract Internal Transactions* propone l'elenco di tutte le transazioni interne. Su Etherscan per transazioni interne si intende quei trasferimenti avvenuti eseguendo un contratto [@stackexchange]. \
...
Le Bundle Transactions pacchetti di una o più transazioni  che passano informazioni alle transazioni successive [@bundletransactions].La pagina *Bundle Transaction* propone un elenco di queste operazioni.

Il link *View Blocks* porta ad una pagina in cui sono elencati tutti i blocchi della catena Ethereum. \

Il link *Forked Blocks* porta ad una pagina in cui sono elencati tutti i forked blocks.
Nel momento in è necessario cambiare le regole possono essere creati dei split temporanei nella network. I nuovi blocchi possono essere generati utilizzando le nuove oppure le vecchie regole. Questi split possono essere temporanei o permanenti [@fork]. Questo elenco esclude i blocchi forked a causa di "Chain Reorganization" [@stackexchange2]. \
...
\
*Token* offre link ai risorse riguardanti i token più popolari.\
...
La Etherscan Token Reputation is designed to help users make informed decisions while interacting with Token Contracts. \
1. Un token ha reputazione OK se : \
2. Is already eligible for 'Neutral' reputation. \
3. Provided sufficient and accurate information. \
4. Clear project goals and communication. \
5. Visible profile of the project founders/backers/advisers. \
6. The token is traded/listed on a major crypto exchange which has AML/KYC checks. \
7. Achieved significant major milestones. \
\
Un token ha reputazione neutra se : \
...
Il link *Token Transfers* propone un elenco di trasferimenti di token ERC-20 [aggiungi dettagli].  \

*NFTs* offre link a risorse riguardanti i NFT. \
![](images\Etherscan_menu_3.png)

Il link *Top NFTs* propone un elenco di collezioni NFT disponibili sulla piattaforma Ethereum ordinati in base al Volume. Per volume si intende la quantità totale di ETH scambiata per quella collezione durante il ciclo di vita di un progetto [@nftvolume]. \
\
Il link *Top Mints* propone un elenco di top NFT mints . \
\
Il link *Latest Trades* propone un elenco di trades di NFT. &Egrafo; possibile specificare il marketplace dove sono avvenuti gli scambi.  \
![](images\Etherscan_nft_trades.png)

Il link *Latest Transfers* propone un elenco di trasferimenti di NFT.  \
[differenza ?]

Il link *Latest Mints* propone un elenco di mint di NFT. \
\
*Resources* offre link a vari servizi come le chat e la newsletter. \
![](images\Etherscan_menu_4.png)

Il link *Charts And Stats* propone un'interfaccia nella quale sono proposti grafi e dashboard interattive. Questi grafi riguardano svariati argomenti: *Market Data*, *Blockchain Data*, *Dashboards*, *Network Data*, *Top Statistics*, *Contracts*. \
...
*Overview* propone un riassunto dei dati riguardanti transazioni, token e network. \
*Transactions* propone le classifiche dei top ETH senders, top ETH receivers, top Txn Count Sent, top Txn Count Received. \
*Token* propone classifiche dei token. \
*Network* propone classifiche degli account in base al gas usato e Txn count. \
*Hot Contracts* propone classifiche dei contratti. \
\
Il link *Leaderboard* propone una classifica dei filtri creati dagli utenti.\
Etherscan permette agli utenti di creare filtri per filtrare i dati disponibili [@advfilter]. \
\
Il link *Directory* propone un elenco di risorse al di fuori della piattaforma Etherscan organizzate in nove categorie. \
...
*API Plans* riporta i possibili piani tariffari e link alla documentazione dell'API. \

*API Documentation* propone link alla documentazione dell'API.  \

*Code Reader* propone un AI che interpreta il codice di uno smart contract e risponde a domande su di esso. A momento (11/03/2025) questo strumento è ancora relativamente nuovo e prima di accedere viene proposta una finestra con un elenco di avvertenze. \
*Code Reader* richiede un account e una chiave API attiva. 

![](images\Etherscan_code_reader.png){height=40%, width=40%}
![](images\Etherscan_code_reader2.png){height=60%, width=60%}
[ prova ad utilizzare il code reader] \
invocato hash di Wrapped USTC \
permette di scegliere tra GPT-3.5, GPT-4, Groq \
...
*Vyper Online Compiler* propone un compilatore online di linguaggio Vyper. &Egrave; possibile specificare la versione da utilizzare. \
\
*Bytecode to Opcode* propone una pagina dove è possibile decodificare il bytecode di basso livello a opcode. \
\
*Broadcast Transaction* propone una pagina dove è possibile trasmettere una Signed Raw Transaction su Ethereum. La transazione deve essere specificata in formato esadecimale. \
\
*More* offre link a servizi miscellanei. La voce *More* ha tre voci principali : *Tools*, *Explore*, *Services*. \
![](images\Etherscan_menu_6.png)

La voce *Tools* propone : \
La pagina *Input Data Decoder* interpreta e analizza i dati mandati ai smart contracts. Bisogna specificare la hash della transazione. \
La pagina *Unit Converter* permette di convertire valori numerici tra Wei e Gwei [cosa sono Wei e Gwei ?]. \
La pagina *CSV Export* permette di scaricare dati in formato csv. &Egrave; possibile specificare che dati scaricare e l'intervallo di tempo con precisione giornaliera. \
La pagina *Account Balance Checker* permette di consultare lo storico del bilancio Ether oppure Token di un account in una data specifica. \
La voce *Explore* propone : \
*Gas Tracker* propone tracking di gas. \
*DEX Tracker* propone le pagine Transactions e Trading Pairs. \
*Node Tracker* Node Tracker shows statistics of detected nodes running on the network. Statistics include the top 10 countries with the highest number of nodes, daily total nodes and node type by clients and OS. \
*Label Cloud* propone Label Word Cloud. \
*Domain Name Lookup* lookup dei Domain Name. \
\
La voce *Services* propone : \
*Token Approvals* : Review and revoke your token approvals for any dApp. \
*Verified Signature* : View, sign and verify message signatures using an Ethereum address. \
*Input Data Messages (IDM)* : Decentralized communication on Ethereum. \
*Advanced Filter* : \
*Blockscan Chat* : \
...
La pagina dune.com/home \

![](images\dune_home.png){height=60%, width=60%}

Lanciando la app si arriva alla pagina Discover. \
\
In alto a sinistra c'è il menu a tendina: *Create*, *Library*, *Discover*, *Metrics*. \
\
*Create* \
...
In alto al centro c'è la barra di ricerca. \

![](images\dune_search.png){height=60%, width=60%}


In alto a destra c'è *Pricing*, *Echo* e *Docs*. \
\
A sinistra ci sono *Categories*, *Feeds* e *Show*. \
...
- mostra messaggi di errore leggibili \

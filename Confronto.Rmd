...
Il link *View Blobs* propone le transazioni di tipo *blob*. Le transazioni *blob* sono state introdotte per permettere transazioni con grandi quantità di dati che persistono nel beacon node per un breve periodo di tempo [@eip4844]. Questi dati non sono accessibili dalla Ethereum Virtual Machine. Si può accedere al loro commitment [@eip4844ethereum]. \
...
Il link *Forked Blocks* porta ad una pagina in cui sono elencati tutti i forked blocks [cosa sono i forked blocks]. Questo elenco esclude i blocchi forked a causa di "Chain Reorganization" [@stackexchange2]. \
...
Il link *Verified Contracts* propone l'elenco dei smart contracts verificati [@smartcontracts].\
\
*Token* offre link ai risorse riguardanti i token più popolari.\
![](images\Etherscan_menu_2.png)

Il link *Top Tokens* propone un elenco di tutti i token ERC-20 [aggiungi dettagli] . \
...
Il link *Top NFTs* propone un elenco di collezioni NFT disponibili sulla piattaforma Ethereum ordinati in base al Volume [che cos'è il Volume?]. \
\
Il link *Top Mints* propone un elenco di ?? . \
\
Il link *Latest Trades* propone un elenco di trades di NFT.  \
...
Il link *Charts And Stats* propone una serie di grafici [..] .\

Il link *Top Statistics* propone un riassunto dei dati disponibili sulla piattaforma [..] . \

Il link *Leaderboard* propone una classifica dei filtri creati dagli utenti [..] . \

Il link *Directory* propone un elenco di risorse organizzate in base al contenuto [..] . \

Il link *Newsletter* propone la newsletter di Etherscan [..] . \

Il link *Knowledge Base* propone la documentazione [..] . \

*Developers* offre link a risorse per sviluppatori. \
![](images\Etherscan_menu_5.png)

*API Plans* : pricing plans e link alla documentazione dell'API. \

*API Documentation* : link alla documentazione dell'API.  \

*Code Reader* : propone un AI che interpreta il codice di uno smart contract e risponde a domande su di esso. \
...
*Smart Contract Search* : Search Smart Contract source codes on Etherscan and filter by contract & deployer addresses, creation date, block number and more. \

![](images\Etherscan_smart_contract_search.png){height=60%, width=60%}

*Contract Diff Checker* : confronta due contratti. \

![](images\Etherscan_contract_diff_checker.png)

*Vyper Online Compiler* : compilatore online di codice Vyper.  \

*Bytecode to Opcode* : attempts to decode the low level Contract Bytecodes to Opcodes. \

*Broadcast Transaction* : This page allows you to paste a Signed Raw Transaction in hex format (i.e. characters 0-9, a-f) and broadcast it over the Ethereum network.\
\
*More* offre link a servizi miscellanei. \
![](images\Etherscan_menu_6.png)

La voce *More* ha tre voci principali : *Tools*, *Explore*, *Services*. \
[alcuni di questi sono ridondanti ?]
*Tools* propone : \
*Input Data Decoder* : Interpret and analyze data sent to Ethereum smart contracts. \
*Unit Converter* : Ether or ETH is often used in different denominations of its currency, such as Wei for interacting with smart contracts and Gwei for calculating gas prices. \
*CSV Export* : permette di scaricare dati. \
*Account Balance Checker* : You can look up the historical balance of an account's Ether or Tokens at a specific block number or date. \
\
*Explore* propone : \

...
I metodi studiati per estrarre informazioni dalla blockchain sono: Blockchain Explorers, Blockchain APIs, Piattaforme tipo SQL e Librerie Web3. Per ogni metodo è stata scelta una piattaforma di esempio. Gli strumenti scelti sono rispettivamente Etherscan, Etherscan API, Dune Analytics, Web3.py. \
Ogni piattaforma è descritta e queste sono confrontate tra loro in base all'usabilità, alle risprse disponibili e ai costi. \
...
![](images\Etherscan_homepage.png){height=40%, width=40%}
...
![](images\Etherscan_filtro.png){height=40%, width=40%}
...
\
![](images\Etherscan_search_results.png){height=40%, width=40%}
...
![](images\Etherscan_menu_1.png){height=40%, width=40%}
...
![](images\Etherscan_menu_2.png){height=40%, width=40%}
...
![](images\Etherscan_menu_3.png){height=40%, width=40%}
...
![](images\Etherscan_menu_4.png){height=40%, width=40%}
...
![](images\Etherscan_chart_stats.png){height=40%, width=40%}
...
![](images\Etherscan_directory.png){height=40%, width=40%}
...
![](images\Etherscan_menu_5.png){height=40%, width=40%}
...
![](images\Etherscan_contract_diff_checker.png){height=40%, width=40%}
...
![](images\Etherscan_menu_6.png){height=40%, width=40%}
...
![](images\Etherscan_label_word_cloud.png){height=40%, width=40%}
\
...
![](images\Etherscan_filter1.png){height=40%, width=40%}
...
![](images\Etherscan_filter2.png){height=40%, width=40%}
...
![](images\Etherscan_mining_calculator.png){height=40%, width=40%}
...
![](images\dune_dashboard.png){height=40%, width=40%}

Nella dashboard di esempio i grafi e la tabella visibile sono gli output di alcune query create dallo stesso account. &Egrave; possibile ottenere output aggiornati rieseguendo le query direttamente sulla pagina. Queste dashboard si possono esportare su GitHub e il loro url si può condividere su altre piattaforme.\
...
![](images\dune_custom_endpoint.png){height=60%, width=60%}
...
- *DEX*: ottiene metatdati e dati statistici per coppie di token.
...
\
La pagina *Transfer Volume* propone un grafo rappresentante gli USD trasferiti su blockchian. Inoltre, viene indicato gli USD trasferiti per ogni blockchain. 
\
La pagina *Transaction Count* propone un grafo rappresentante il numero di transazioni su blockchain. &Egrave; riportata una tabella con il numero di transazioni per ogni blockchain.
...
Queste voci permettono di fare ricerche all'interno di Dune Analytics. *Categories* permette di scelgliere cosa cercare. *Feeds* permette di filtrare i risultati in qualche modo. *Show* permette di scegliere se vedere query, dashboard o entrambe. \
...
![](images\Infura_home.png){height=40%, width=40%}
...
Per esaminare e confrontare i diversi strumenti disponibili sono stati scelti tre parametri di confronto: \
...
Questi sono analizzati separatamente.
...
&Egrave; stato riscontrato un problema visualizzando gli output di una query: la tabella con i risultati risultava compressa e non era possibile leggere le righe. La pagina propone un handle di ridimensionamento, ma è stato necessario prima fare zoom out. \
...
La libreria Web3.py è facile da installare e non richiede altre librerie per funzionare. &Egrave; disponibile una documentazione ufficiale. Le funzioni utilizzano nomi che comunicano in modo chiaro a cosa servono. Permette di scrivere codice espressivo e ordinato. I messaggi di errore sono piuttosto chiari. 
...
Etherscan sembra essere la piattaforma più accessibile e facile da utilizzare. &Egrave; la più adatta per un utente che svolge attività sulla catena Ethereum. Nonostante non permetta di svolgere mining e staking direttamente essa può fornire risorse di ricerca utili per chi si occupa di staking oppure mining. Esso permette la creazione di filtri (può rendere più facile il monitoraggio). \
Per uno sviluppatore la piattaforma non fornisce molto supporto, ci sono alternative più versatili. \
Per un team il supporto offerto da Etherscan è adeguato. \
\
Etherscan API invece risulta meno accessibile ad un utente senza competenze informatiche. Per gli utente individuale, i minatori e i team potrebbe preferire utilizzare Etherscan. Per chi sviluppa invece l'output in formato JSON può risultare molto utile per ottenere dati direttamente nella propria applicazione. \
\
Su Dune sono disponibili informazioni su decine di catene, non solo quelle legate all'ecosistema Ethereum. Esso richiede competenze di SQL. Questa potrebbe essere una buona l'opzione per chi si occupa di analisi e ricerca, per chi fa mining e per i team. In particolare, la creazione di dashboard e di custom endpoints permette di monitorare parametri facilmente. La piattaforma che offre maggiore supporto all'attività di analisi risulta essere Dune Analytics. I dati disponibili sono di interesse anche per gli utenti individuali, ma richiede minime competenze di SQL. &Egrave; fornito supporto sotto forma dell'AI ed è possibile utilizzare le query create da altri utenti. \
\
C'è una grossa differenza nel supporto fornito da Web3 e quello fornito dalle altre piattaforme. La libreria Web3.py sembra essere più che sufficiente per chi programma dApp. Web3.py può essere utilizzato per scrivere programmi per il monitoraggio. Web3.py è l'unica piattaforma che permette di svolgere la transazione direttamente, ma si occupa solo di alcune catene. \
Web3 può essere uno strumento molto potente per chi ha competenze avanzate di programmazione in tutti i gruppi. L'unico limite è che la libreria si occupa solo dell'ecosistema Ethereum.
...
6. **Gli strumenti che permettono di visualizzare i dati dei singoli blocchi.**
7. **Gli strumenti che permettono di visualizzare i dati dei token.**
8. **L'accesso alle risorse esterne.**
...
Etherscan crea delle classifiche dei token ERC-20 e di collezioni di NFT (*Top Token*, *Top NFT*). Su i token ERC-20 in particolare, Etherscan propone delle sue valutazioni, chiamate reputazione. La pagina *Top Statistics* riassume i dati disponibili sulla piattaforma.
...
- il *Code Reader*, che interpreta il codice degli smart contracts.
- il *Similar Contract Search*, che permette di cercare smart contract con codice simile a quello di un contratto specifico.
- il *Smart Contract Search*, cercare il codice sorgente di smart contract in base al contratto e all’indirizzo del deployer, la data di creazione, il numero del blocco e altro.
- il *Account Balance Checker*, consultare lo storico del bilancio Ether oppure Token di un account.
- il *Domain Name Lookup*, permette di cercare un indirizzo utilizzando il Domain Name Lookup.
...
Data la pagina di un contratto o di un token è possibile interagire con il smart contract associato andando alle voci *Read Contract* e *Write Contract*. Queste due pagine permettono di eseguire le funzioni scritte nei contratti. Con le funzioni di *Read Contract* è possibile ottenere informazioni già presenti nel codice. Con *Write Contract* invece è possibile utilizzare le funzioni che richiedono il proprio wallet. \
*Verify Contracts* permette di verificare e pubblicare codice sorgente di un smart contract. 

*Gli strumenti che permettono di visualizzare i dati dei singoli blocchi.* \
Ogni blocco della catena ha una pagina dedicata con informazioni dettagliate. 
  
*Gli strumenti che permettono di visualizzare i dati dei token.* \ 
Ogni token ha una sua pagina dedicata con informazioni dettagliate. 

*L'accesso alle risorse esterne.* \
Etherscan raccoglie e organizza numerosi link ha supporto esterno sotto la voce *Directory*. Queste voci sono *Exchanges*, *Wallet*, *Listing and Prices*, *News and Forums*, *Events*, *Learning Resources*, *Smart Contracts*, *Mining*, *Others*.

<!--
Etherscan mantiene dati aggiornati e li organizza in pagine fornendo feedback su i dati stessi, come ad esempio i rating dei token.

Molti di questi riguardano gli smart contract (*Verify Contracts*, *Similar Contract Search*, *Smart Contract Search*, *Contract Diff Checker*, *Vyper Online Compiler*).
&Egrave; fornito supporto AI sotto forma di *Code Reader* ( interpreta il codice di uno smart contract e risponde a domande su di esso).
Sono forniti strumenti miscellanei che permettono di rielaborare le informazioni messe a disposizione dalla piattaforma.
Sono raccolti e organizzati molti link a risorse esterne di interesse. 
&Egrave; possibile scaricare i dati.

Come visto prima, Etherscan fornisce una vasta gamma di strumenti per rielaborare i dati. Complessivamente, questi strumenti sono facili da utilizzare. 
-->

#### Etherscan API

<!--
Attualmente (01/04/2025) esistono due versioni di Etherscan API: con la prima versione è necessario specificare l'ID della catena. Con la seconda invece le chiavi sono state unificate e possono essere utilizzate su tutte le catene. L'elenco delle catene disponibili è riportato in appendice.

[spiega bene]

Dai API endpoint messi a disposizione, è possibile ottenere in output un numero illimitato di [spwcificare meglio], tranne per alcuni endpoint che hanno un limite di 10000. \
I dati sono aggiornati. &Egrave; possibile scaricare i dati ma non è possibile rielaborarli direttamente.
...
*Le catene blockchain a cui la piattaforma da accesso.* \
Etherscan API ha accesso a circa sessanta catene. Queste sono elencate in appendice. Attualmente (01/04/2025) esistono due versioni di Etherscan API (V1 e V2) che differiscono per quali catene sono supportate.
...
*Gli strumenti per la ricerca dei dati.* \
Etherscan API permette l'utilizzo di endpoint.

*Gli strumenti per il download dei dati.* \
Non direttamente: è possibile scaricare i dati in formato JSON scrivendo del proprio codice. 
...
*L'accesso alle risorse esterne.*\
Etherscan API non offre diretto accesso a risorse esterne.


#### Dune

<!--
Dune propone i dati di numerose blockchain. I dati sono completi e aggiornati. L'elenco delle catene è riportato in appendice. \
...
\
Con le Dashboard è possibile visualizzare informazioni complesse in modo intuitivo e accattivante. I Team possono svolgere attività in gruppo. Al di fuori dei team, è possibile utilizzare le query create da altri utenti.\
...
- la pagina *Transaction fees* propone un grafo rappresentante i USD spesi in gas
...
- è possibile creare dei custom endpoints
  
*Gli strumenti per il download dei dati.* \
&Egrave; possibile scaricare dati in formato csv solo con alcuni piani tariffari.

*Gli strumenti per rielaborare i dati.* \
Oltre a creare grafi, non è possibile rielaborare gli output delle query direttamente sulla piattaforma.

*Gli strumenti che permettono l'interazione con gli smart contracts.* \
Non è possibile interagire direttamente con gli smart contract.
  
*Gli strumenti che permettono di visualizzare i dati dei singoli blocchi.* \
&Egrave; possibile accedere ai dati dei singoli blocchi tramite delle query.

*Gli strumenti che permettono di visualizzare i dati dei token.* \
&Egrave; possibile accedere ai dati dei singoli token tramite delle query.
...
<!-- La libreria Web3.py permette di interagire direttamente con la blockchain, di conseguenza permette di ottenere dati aggiornati. Inoltre la libreria raccoglie un insieme esteso di funzioni per monitorare e svolgere operazioni su alcune catene blockchain. Essa permette la creazioni di nuovi blocchi e nuovi wallet. Permette inoltre operazioni piuttosto avanzate. \
Inoltre è possibile estendere la funzionalità creando delle proprie funzioni Python che utilizzano la libreria. \
-->

**Le catene blockchain a cui la piattaforma da accesso.** \
Web3.py ha accesso alla catena Etherscan.
...
**Gli strumenti per la ricerca dei dati.** \
Le funzioni proposte da Web3.py permettono di accedere ai dati.

**Gli strumenti per il download dei dati.** \
Non direttamente, tutti gli output funzioni si possono scaricare.

**Gli strumenti per rielaborare i dati.** \
Esistono funzioni per rielaborare i dati.

**Gli strumenti che permettono l'interazione con gli smart contracts.** \
Esistono funzioni per interagire con gli smart contract.

**Gli strumenti che permettono di visualizzare i dati dei singoli blocchi.** \
Esistono funzioni per ottenere i dati dei singoli blocchi.

**Gli strumenti che permettono di visualizzare i dati dei token.** \
Esistono funzioni per ottenere i dati dei token.
...
- Tutte le piattaforme offrono dati aggiornati. Ogni piattaforma ha accesso ha catene diverse.
- Gli strumenti disponibili cambiano in base alla piattaforma. 
- La precisione dei dati cambia in base alla piattaforma.
- Etherscan fornisce feedback sui dati, le altre piattaforme no
- Etherscan API interagisce con più catene rispetto a Etherscan
- Dune interagisce molte catene
- Web3.py è un po' limitato
- Etherscan e Dune permetto di rielaborare i dati, le altre piattaforme no
- Dune non permette di scaricare i dati senza pagare, le altre piattaoforme sì

1. **Le catene blockchain a cui la piattaforma da accesso.** -> cambia con ogni piattaforma
2. **Le informazioni disponibili**, riguardanti: tutti
  - transazioni
  - indirizzi
  - blocchi
  - token
  ...
  - gas
  - eventi (log)
3. **Le informazioni disponibili a seguito di rielaborazione dei dati delle piattaforme.** -> Etherscan e Dune rielaborano mentre API e Web3 no 
4. **Gli avvisi e le notifiche generati dalla piattaforma.** tutti, tranne API
...
Esistono quattro piani. Sono sempre disponibili crediti, memoria e un certo numero di datapoints per credit. Per datapoints si intende i singoli valori nelle tabelle.
Quello che cambia con ogni piano è la quantità di risorse disponibili. Inoltre sono aggiunti ulteriori servizi.

Analyst aggiunge query management endpoints.

Plus aggiunge large query engine, csv export.

Premium aggiunge private query views, private materialized views, private data uploads.
...
![](images\Infura_cost.png){height=40%, width=40%}
...
*Enterprise* offre molta più funzionalità, ma bisogna contattare Infura per definire i dettagli.


#### Confronto

Si confrontano le offerte disponibili con Etherscan, Dune Analytics e Infura. \
...
Le funzionalità di Etherscan (API) e Infura disponibili gratuitamente sono molto estese. Con Dune invece non poter scaricare i dati può essere molto limitante. \
...
Etherscan offre il numero più elevato di chiamate. Dune è l'unica piattaforma che offre storage e permette di generare un numero illimitato di chiavi.\
Dune e Infura non forniscono direttamente il numero di chiamate, perchè il numero di crediti dipende da cosa viene invocato. \
In particolare, con Dune: 
...
Riguardo Infura, la pagina specifica il costo in crediti per quattro funzioni :
...
Il costo di Dune è stato approssimato come 10 crediti per chiamata, il costo di Infura è stato approssimato come 255 crediti a chiamata.
...
| Infura | 50 | 58823 | 15 millioni  | not specified | 0 | 5 |
"
cat(paid1_vers)

```

Etherscan è l'opzione più costosa e fornisce il numero di chiamate più elevato ma il numero di chaivi più basso.
Etherscan offre supporto.
Dune offre memoria.
...
Etherscan resta l'unica piattaforma che non offre un numero illimitato di chiavi.
Dune ha il costo più elevato.
...
Dune ha un costo molto più elevato delle altre opzioni. Unico tier che permette di scaricare i dati.
Etherscan offre un numero più elevato di chiamate.
Infura è l'unica permette di specificare costo custom.
...
- Le diverse piattaforme hanno permesso di ottenere risultati diversi
- Per un analisi più completa potrebbe essere di interesse combinare i risultati di Etherscan e Dune
- Web3 di minore interesse
- Etherscan preferibile ad Etherscan API

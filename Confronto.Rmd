...
I Blockchain Explorers forniscono interfacce pubbliche che permettono di ottenere informazioni sui dati della blockchain. L'interfaccia studiata è Etherscan. Etherscan offre numerose funzionalità, come strumenti di tracking, interazione con gli smart contracts e un blockchain explorer. [@docsetherscan]
...
\
La piattaforma Etherscan è composta da svariate pagine. Si accede alla piattaforma tramite la pagina home. La pagina home in particolare indica chiaramente agli utenti in che direzione procedere per svolgere l'attività di interesse. Essa offre direttamente informazioni riguardo alla cryptovaluta ETH, mentre per altre attività ci sono dei menù a tendina e una barra di ricerca. \
\
...
Se ci sono delle possibili ambiguità riguardo delle voci, allora vengono indicati degli *i* che forniscono informazioni aggiuntive. \
\
&Egrave; possibile andare avanti e indietro nelle pagine. Come già accennato, esiste una porzione della pagina che rimane sempre uguale. Questa contiene la barra di ricerca e il logo di Etherscan: cliccando sul logo si ritorna alla home page. \
\
Complessivamente, Etherscan ha una elevata usabilità.
...
Inoltre è possibile accedere alla documentazione ufficiale di Etherscan. 
...
Si visualizzano i costi in termini assoluti: \
...
ggplot(etherscan_api, aes(x=c(0, 1, 2, 3, 4), y=cost)) +
  ...
  scale_x_continuous(label =  c("FREE", "STANDARD", "ADVANCED", "PROFESSIONAL", "PRO_PLUS"))
...
Come variano i costi delle singole chiamate al secondo ? Si riportano : \
\
differenza costo tariffe consecutive / differenza numero chiamate tariffe consecutive \

```{r cost_sec, echo = FALSE, result=TRUE, warning=FALSE}

standard_cost_sec <- etherscan_api$cost[2]/( etherscan_api$per_sec[2] - etherscan_api$per_sec[1] )

advanced_cost_sec <- ( etherscan_api$cost[3] - etherscan_api$cost[2] )/( etherscan_api$per_sec[3] - etherscan_api$per_sec[2] )

professional_cost_sec <- ( etherscan_api$cost[4] - etherscan_api$cost[3] )/( etherscan_api$per_sec[4] - etherscan_api$per_sec[3] )

cost_sec <- c(0, standard_cost_sec, advanced_cost_sec, professional_cost_sec) 
cost_sec <- data.frame(cost_sec)

ggplot(cost_sec, aes(x=c(0, 1, 2, 3), y=cost_sec)) +
  geom_line() +
  geom_point() +
  xlab("") +
  ylab("costo per chiamate aggiuntive") +
  scale_x_continuous(label =  c("FREE", "STANDARD", "ADVANCED", "PROFESSIONAL"))

```

Come variano i costi delle singole chiamate al giorno ? Si riportano : \
\
differenza costo tariffe consecutive / differenza numero chiamate tariffe consecutive \

```{r advanced_cost_day, echo = FALSE, result=TRUE, warning=FALSE}


standard_cost_day <- etherscan_api$cost[2]/( etherscan_api$per_day[2] - etherscan_api$per_day[1] )

advanced_cost_day <- ( etherscan_api$cost[3] - etherscan_api$cost[2] )/( etherscan_api$per_day[3] - etherscan_api$per_day[2] )


professional_cost_day <- ( etherscan_api$cost[4] - etherscan_api$cost[3] )/( etherscan_api$per_day[4] - etherscan_api$per_day[3] )

pro_professional_cost_day <- ( etherscan_api$cost[5] - etherscan_api$cost[4] )/( etherscan_api$per_day[5] - etherscan_api$per_day[4] )


cost_day <- c(0, standard_cost_day, advanced_cost_day, professional_cost_day, pro_professional_cost_day) 

cost_day <- data.frame(cost_day)

ggplot(cost_day, aes(x=c(0, 1, 2, 3, 4), y=cost_day)) +
  geom_line() +
  geom_point() +
  xlab("") +
  ylab("costo per chiamate aggiuntive") +

...
- **eth_blockNumber** : restituisce il numero del blocco più recente.
- **eth_getBlockByNumber** : ottiene informazioni riguardo un blocco dato il suo numero.
- **eth_getUncleByBlockNumberAndIndex** : ottiene informazioni di un blocco Uncle in base al suo numero.
- **eth_getBlockTransactionCountByNumber** : restituisce il numero di transazioni in un blocco.
- **eth_getTransactionByHash** : restituisce le informazioni riguardo una transazione dato il suo hash.
- **eth_getTransactionByBlockNumberAndIndex** : restituisce le informazioni riguardo una transazione dato il numero del suo blocco e l'indice di posizione della transazione.
- **eth_getTransactionCount** : restituisce il numero di transazioni effettuate da un indirizzo.
- **eth_sendRawTransaction** : invia una transazione pre firmata per broadcast sulla rete Ethereum.
- **eth_getTransactionReceipt** : restituisce il receipt di una transazione dato il hash della transazione.
- **eth_call** : esegue una nuova message call immediatamente senza creare una transazione sulla blockchain.
- **eth_getCode** : restituisce il codice ad un dato indirizzo. 
- **eth_getStorageAt** : restituisce il valore archiviato ad un dato indirizzo.
- **eth_gasPrice** : restituisce il prezzo attuale del gas espresso in wei.
- **eth_estimateGas** : effettua una chiamata o transazione che non sarà aggiunta alla blockchain e restituisce il gas utilizzato.
...
Oltre a fornire i dati, Etherscan li interpreta per renderli più comprensibili agli utenti.
Come visto vengono proposte valutazioni e classifiche dei token e statistiche dei dati. 
Esistono strumenti di ricerca per navigare le risorse messe a disposizione sulla piattaforma, come ad esempio la barra di ricerca. 
...
La libreria Web3.py riguarda solo la catena Ethereum. Ha funzionalità molto potenenti, dato che permette di accedere ai dati, svolgere operazioni sulla catena e gestire il *Middleware*.
...
Esistono quattro piani. Sono sempre disponibili crediti, memoria e un certo numero di datapoints per credito. Per datapoints si intende il numero di righe nelle tabelle di output.
...
Il piano *Premium* aggiunge le private query views, private materialized views, private data uploads.
[ cerca ]

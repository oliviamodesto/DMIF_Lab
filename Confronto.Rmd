...
- *Eigenlayer* : fornisce informazioni riguardo i metadati di AVS, gli operatori e la mappatura di metadati agli operatori.
- *Projects* : fornisce dati riguardanti Linea LXP.
- *Contracts*: fornisce informazioni riguardo i contratti lanciati su una catena EVM basata sull'attività degli ultimi 30 giorni.
...
\
La pagina *Transaction fees* propone un grafo rappresentante gli USD spesi in gas. Inoltre, viene indicato il transaction fee per ogni blockchain. In fondo alla pagina viene descritta la metodologia utilizzata per il calcolo. \
\
...
Dune fornisce un'AI per assistere con la creazione o modifica delle query. Se qualcosa non va a buon fine, l'interfaccia produce dei messaggi di errore comprensibili. \
...
Il progetto si riferisce alla versione v7.10.0 della libreria. 
...
&Egrave; possibile configurare il middleware. Per middleware si intende uno strato di software che sta tra i provider e Web3. In particolare è possibile aggiungere, rimuovere, modificare e cancellare strati di middleware a runtime.
...
Riguardo i contratti, Web3.py fornisce una funzionalità molto completa. &Egrave; possibile compilare un contratto Solidity e fare il deploy. &Egrave; possibile creare le Contract Factories, ovvero oggetti che permettono di interfacciarsi con i smart contracts oppure fare il deploy di smart contract. Inoltre, è possibile interagire con gli eventi dei contratti. Infine, Web3.py fornisce un'API per invocare le funzioni di un contratto.
...
La libreria Web3.py è uno strumento molto potente per chi sviluppa dApp. Essa richiede competenze avanzate e non è adatta ad un utente casuale.
...
- i **costi** ovvero i piani tariffari
...
- **Team di progetti riguardanti DeFi, oppure NFT, oppure Token** : Le piattaforme di finanza decentralizzata vorranno poter monitorare la liquidità dei token, le transazioni e le statistiche dei pool di liquidità. Chi crea NFT vorrà poter creare l'NFT, fare mining, fare operazioni e monitorare lo stato. I team ICO vorranno poter lanciare i token o airdrop, monitorare la distribuzione dei token, verificare le interazioni con gli smart contract e gestire la tokenomica del progetto.
  ...
  Per tokenomica si intende lo studio e l’analisi di aspetti economici di una criptovaluta o progetto blockchain. Esempi di informazioni sono il numero totale di token in circolazione, i meccanismi di distribuzione, i meccanismi di minting e burning. \
...
- **Minatori e Validatori** : \
  ...
  *Un minatore o validatore ha accesso a strumenti per effettuare lo staking ?* No. \
  *Un minatore o validatore ha accesso a strumenti per effettuare il mining ?* No. \
  *Un minatore o validatore può stimare il slashing risk ?* Sì, se i dati sono messi a disposizione. \
  *Un minatore o validatore può conoscere la validator uptime ?* No. \
  *Un minatore o validatore può stimare la reward rate ?* Sì, se questa è stata documentata.\
  *Un minatore o validatore può ottenere informazioni sulla liquidity ?* Sì, se questa è stata documentata.\
...
- **Minatori e Validatori** : \
  *Un minatore o validatore può monitorare la produzione di blocchi ?* Sì, si possono invocare singoli blocchi, utilizzando il loro  numero oppure una parola chiave (*latest*, *earliest*, *pending*, *safe*, *finalized*). \
  *Un minatore o validatore può accedere alla mining pool ?* No. \
...
Etherscan è la piattaforma più accessibile e facile da utilizzare. &Egrave; anche quella più ricca di informazioni riguardo l'ecosistema Ethereum. Permette attività di monitoraggio tramite la creazione di filtri. &Egrave; la più adatta ad un utente che svolge attività sulla catena Ethereum. Non permette di svolgere mining e staking direttamente, ma può fornire risorse utili ai minatori oppure ai stakers. \
...
\
Su Dune sono disponibili informazioni su decine di catene, non solo quelle legate all'ecosistema Ethereum. Non richiede competenze di programmazione, ma richiede la capacità di scrivere query SQL. &Egrave; fornito supporto sotto forma di AI. Inoltre, è possibile utilizzare le query create da altri utenti.\
...
\
C'è una grossa differenza nel supporto fornito da Web3 e quello fornito dalle piattaforme. Web3 può essere uno strumento molto potente per chi ha competenze avanzate di programmazione in tutti i gruppi. Un limite è che si occupa solo dell'ecosistema Ethereum. \
La libreria Web3.py sembra essere più che sufficiente per chi programma dApp. \
Web3.py è l'unica piattaforma che permette di svolgere la transazione direttamente, ma si occupa solo di alcune catene.
...
- il *Account Balance Checker*, che permette di consultare lo storico del bilancio Ether oppure Token di un account.
...
*L'accesso alle risorse esterne.* Sì. \
...
Tutti i metodi forniscono dati completi e aggiornati. Ci sono alcune differenze tra le risorse e gli strumenti messi a disposizione.

Oltre a fornire i dati, Etherscan li interpreta per renderli più comprensibili agli utenti. Esistono strumenti di ricerca per navigare le risorse messe a disposizione sulla piattaforma. Inoltre sono forniti molti strumenti riguardano gli smart contract. L'unico limite è che Etherscan si occupa solo di quattro catene.

Etherscan API invece non rielabora i dati in nessun modo: l'interpretazione è affidata interamente agli utenti.
Sono forniti gli endpoint da cui scaricare i dati, e non ci sono strumenti di rielaborazione.

Dune propone alcune interpretazioni dei dati [PAGINE DEDICATE ?]. Complessivamente, Dune dedica meno attenzione agli smart contracts rispetto a Etherscan. Sono forniti strumenti di ricerca per naviagre le risorse messe a disposizione. Sono forniti strumenti per visualizzare i dati in modi più comprensibili. A differenza di Etherscan, Etherscan API e Web3.py Dune si occupa anche di catene al di fuori dell'universo Ethereum. 

La libreria Web3.py riguarda solo la catena Ethereum. Ha una funzionalità molto potenente, ma la rielaborazione, visualizzazione e interpretazione dei dati è interamente affidata agli utenti. 
...
Etherscan fornisce direttamente il numero di chiamate. Questo è stato utilizzato per l'analisi.\
Dune e Infura invece forniscono un numero di crediti: questi crediti sono consumati effettuando chiamate. Diverse chiamate hanno costi diversi. \
Per Dune il costo default di una esecuzione programmata di una query è fissato a 10 crediti. Questo è stato scelto come approssimazione del costo di una chiamata.
<!--
...
Il costo di una chiamata è stato approssimato a quello di una eth_call.
...
Etherscan offre il numero più elevato di chiamate. Dune è l'unica piattaforma che offre storage e permette di generare un numero illimitato di chiavi. Con questa versione nonè possibile scaricare i dati da Dune.
...
Etherscan fornisce il numero di chiamate più elevato ma il numero di chaivi più basso. L'insieme dei endpoint raggiungibili è ampliato. 
...
Dune resta l'unica piattaforma ad offrire spazio di archiviazione e un numero illimitato di chiavi, ma ha il numero di chiamate più basso. 
...
Etherscan offre il numero più elevato di chiamate delle tre, ma solo tre chiavi API.
...
La piattaforma Dune risulta avvantaggiata perché si occupa anche di ecosistemi non Ethereum. I dati raccolti hanno una precisione più elevata rispetto ad Etherscan. Mancano però i dati del market cap, che la piattaforma sceglie di escludere. Non è stato possibile consultare il white paper della valuta di interesse.

Con Web3.py sono state verificate meno informazioni rispetto ad altre piattaforme. &Egrave; lo strumento meno adatto al caso di studio.

Per uno studio completo potrebbe essere di interesse combinare i risultati di più piattaforme, come Etherscan e Dune

## Conclusioni

In questo progetto sono stati studiati quattro strumenti per interagire con le blockchain. Ongi strumento rappresenta un approcio differente: Etherscan rappresenta i Blockchain Explorer, Etherscan API rappresenta le Blockchain API, Dune Analytics rappresenta le piattaforme SQL, Web3.py rappresenta le librerie Web3. \

Queste piattaforme sono state descritte una ad una. Esse sono state valutate in base a usabilità, risorse disponibili, e costi. Per ogni parametro di valutazione le piattaforme sono state confrontate. \
